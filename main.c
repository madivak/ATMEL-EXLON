...
unsigned char CheckSMS()
{
	...
	{	status[0] = 5; status[1] = status[2] = status[3] = 0; printf("AT+CMGD=1,2\r\n"); _delay_ms(2000);} 
	...
	else
	{
		...
		if (byteGPS==13)
		{
			...
			if (x==1 && y==1)
			{
				...
				free(GGA); //The memory locaction pointed by GGA is freed. Otherwise it will cause error
				free(RMC); //The memory locaction pointed by RMC is freed. Otherwise it will cause error
				free(Digits);
				_delay_ms(2000);
			}
			else{}
